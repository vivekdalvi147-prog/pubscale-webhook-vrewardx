const { admin, db, rtdb, firebaseInitialized } = require('./firebase');
...
module.exports = async (req, res) => {
  ...
  try {
    ...
    // 2. SECURE RTDB-BACKED RATE LIMITING (Max 20 requests per minute)
    const rateLimitSnapshot = await rtdb.ref(`users/${uid}/ai_limit/timestamps`).get();
    let timestamps = [];
    if (rateLimitSnapshot.exists()) {
      timestamps = rateLimitSnapshot.val() || [];
    }
    ...
    timestamps.push(now);
    await rtdb.ref(`users/${uid}/ai_limit/timestamps`).set(timestamps);
...
- Crucial support behavior: Whenever a user mentions any issue with PubScale offers, coin tracking, cashout delays, task rewards, or when they are confused about how to complete an action, you MUST proactively ask them to take a screenshot and upload / attach it using the '+' (gallery picker) or camera screen capture buttons in the chat. Explain to them that you are a multimodal AI and can visually read and inspect their screenshots to identify any issues and guide them on what went wrong or how to solve it immediately!
