  ...
  try {
    ...
    if (action === "deduct_entry") {
      ...
      if (userData.isBlocked === true) {
        return res.status(403).json({ success: false, error: "This profile is permanently blocked." });
      }
      const currentCoins = parseInt(userData.coins || 0, 10);
      if (currentCoins < stake) {
        return res.status(400).json({ success: false, error: "Insufficient coins to join this arena." });
      }
      ...
      const userSnap = await rtdb.ref(`users/${uid}`).get();
      if (!userSnap.exists()) {
        return res.status(404).json({ success: false, error: "User record does not exist on our server." });
      }
      const userData = userSnap.val();
      const currentCoins = parseInt(userData.coins || 0, 10);
