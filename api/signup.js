...
function isRateLimited(ip, endpoint, limit = 15, windowMs = 60000) {
...
module.exports = async (req, res) => {
  ...
  // Rate Limiting Check
  if (isRateLimited(clientIp, "signup", 15, 60000)) {
  ...
  try {
    // 1. Verify standard Firebase Auth ID Token securely on the server side WITH revoked checks
    ...
    if (!userSnapshot.exists()) {
      ...
      // Anti-cheat verification 3: IP Duplication check
      try {
        const ipUsersSnap = await rtdb.ref("users").orderByChild("ipAddress").equalTo(clientIp).get();
        if (ipUsersSnap.exists()) {
          const existingCount = Object.keys(ipUsersSnap.val()).length;
          if (existingCount >= 2) {
            // Block start balance bonus to prevent multi-registration welcome farm
            userCoins = 0;
          }
        }
      } catch (errIp) {
        console.warn("Could not execute IP filter verification:", errIp.message);
      }
      ...
      let isUnique = false;
      let checkAttempts = 0;
      while (!isUnique && checkAttempts < 15) {
        let code = '';
        for (let i = 0; i < 5; i++) {
      ...
      if (userCoins > 0) {
        ...
        await rtdb.ref(`transactions/${uid}_WELCOME`).set(welcomeTxObj);
      }
    } else {
    ...
    return res.status(200).json({
      ...
      uid: uid,
      isNewUser: isNewUser,
      coins: finalData.coins !== undefined ? finalData.coins : userCoins,
    ...
    return res.status(500).json({ success: false, error: "An internal server error occurred while processing registration." });
  }
};
